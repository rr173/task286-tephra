...
func TestListBatchesSeesFreshObservationCount(t *testing.T) {
	svc := newProbeSvc(t)
	b, err := svc.CreateBatch(&model.Batch{Name: "KR-L1", TopDepth: 1.0, BottomDepth: 2.0})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := svc.ListBatches(); err != nil {
		t.Fatal(err)
	}
	for i := 0; i < 3; i++ {
		o := probeObs(string(rune('A'+i)), 0)
		o.BatchID = b.ID
		if _, _, err := svc.ImportObservation(&o); err != nil {
			t.Fatal(err)
		}
	}
	list, err := svc.ListBatches()
	if err != nil {
		t.Fatal(err)
	}
	for _, row := range list {
		if row.ID == b.ID {
			if row.ObservationCount != 3 {
				t.Fatalf("observation_count=%d want=3", row.ObservationCount)
			}
			return
		}
	}
	t.Fatalf("batch %s not listed", b.ID)
}
