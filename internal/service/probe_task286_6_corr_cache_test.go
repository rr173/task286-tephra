...
func TestListCorrelationsSeesFreshCreate(t *testing.T) {
	svc := newProbeSvc(t)
	up := seedComparableBatch(t, svc, "KR-C1", 40.0, 40.8)
	lo := seedComparableBatch(t, svc, "KR-C2", 41.0, 41.9)
	if _, err := svc.ListCorrelations(); err != nil {
		t.Fatal(err)
	}
	...
	t.Fatalf("correlation %s missing from cached list len=%d", c.ID, len(list))
}
