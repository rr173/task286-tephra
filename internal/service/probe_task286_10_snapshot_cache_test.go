...
func TestGetSnapshotSeesPublishedStatus(t *testing.T) {
	svc := newProbeSvc(t)
	up := seedComparableBatch(t, svc, "KR-S10A", 90.0, 90.8)
	lo := seedComparableBatch(t, svc, "KR-S10B", 91.0, 91.9)
	...
	snap, err := svc.CreateSnapshot("pub-cache", "probe")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := svc.GetSnapshot(snap.ID); err != nil {
		t.Fatal(err)
	}
