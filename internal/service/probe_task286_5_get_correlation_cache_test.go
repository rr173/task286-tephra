...
func TestGetCorrelationSeesAdjudicatedStatus(t *testing.T) {
	svc := newProbeSvc(t)
	up := seedComparableBatch(t, svc, "KR-F1", 30.0, 30.8)
	lo := seedComparableBatch(t, svc, "KR-F2", 31.0, 31.9)
	c, err := svc.CreateCorrelation(up.ID, lo.ID)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := svc.GetCorrelation(c.ID); err != nil {
		t.Fatal(err)
	}
	if _, err := svc.AdjudicateCorrelation(c.ID, "confirmed", "r", "snap"); err != nil {
		t.Fatal(err)
	}
	got, err := svc.GetCorrelation(c.ID)
	if err != nil {
		t.Fatal(err)
	}
	if got.Status != model.CorrConfirmed {
