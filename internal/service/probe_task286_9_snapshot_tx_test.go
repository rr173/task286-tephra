...
func TestCreateSnapshotRollbackOnLinkFailure(t *testing.T) {
	svc := newProbeSvc(t)
	st := svc.Store()
	snap := &model.Snapshot{
		...
		Links: []model.SnapshotLink{
			{CorrelationID: "co-dup", UpperBatchID: "b1", LowerBatchID: "b2", Status: "confirmed", Distance: 1, PValue: 0.5},
			{CorrelationID: "co-dup", UpperBatchID: "b1", LowerBatchID: "b3", Status: "confirmed", Distance: 2, PValue: 0.4},
		},
	}
	if _, err := st.CreateSnapshot(snap); err == nil {
		t.Fatal("expected duplicate snapshot link insert to fail")
	}
	if _, err := st.GetSnapshotByName("tx-rollback-probe"); err == nil {
		t.Fatal("orphan snapshot head persisted after link failure")
	}
}
