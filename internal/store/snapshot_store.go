...
func (s *Store) PublishSnapshot(id string) (*model.Snapshot, error) {
	...
	if _, err := s.db.Exec(`DELETE FROM snapshot_links WHERE snapshot_id=?`, id); err != nil {
		return nil, err
	}
	corrs, err := s.ListCorrelations()
	if err != nil {
		return nil, err
	}
	for _, c := range corrs {
		...
		if _, err := s.db.Exec(
			`INSERT INTO snapshot_links(snapshot_id, correlation_id, upper_batch_id, lower_batch_id, status, distance, p_value, strat_gap)
			 VALUES(?,?,?,?,?,?,?,?)`,
			id, c.ID, c.UpperBatchID, c.LowerBatchID, string(c.Status), c.Distance, c.PValue, c.StratGap); err != nil {
			return nil, err
		}
	}
