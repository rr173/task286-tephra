...
func (s *Store) SaveCorrelationResult(c *model.Correlation) error {
	...
	_, err := s.db.Exec(
		`UPDATE correlation_links SET status=?, distance=?, dof=?, p_value=?, compatible=?, strat_ok=?, strat_gap=?, strat_note=?, standard_version=?, updated_at=? WHERE id=?`,
		string(c.Status), c.Distance, c.DOF, c.PValue, compatible, stratOK, c.StratGap, c.StratNote,
		c.StandardVersion, c.UpdatedAt, c.ID)
	if err != nil {
		return err
	}
